...
class EHFNet(nn.Module):
    ...
    def forward(self, data: HeteroData, t: Tensor) -> EHFNetOutput:
        ...
        # 旋转：体帧 MLP → 世界帧等变角速度
        # 原理：直接聚合 r × v 近似角动量 L，但训练目标是角速度 ω = I⁻¹L，对非球形分子存在系统性偏差。改用主惯量帧变换：
        #   1. compute_principal_frame → R_frame [B,3,3]，列向量为主轴方向
        #   2. rot_body_head(h_mol) → ω_body [B,3]，体帧角速度（旋转不变量）
        #   3. v_rotation = R_frame @ ω_body，世界帧等变角速度
