...
from ehfnet.data.featurizers.chemistry import (
    Element,
    ResidueType,
    resolve_protein_residue_type,
)
...
from ehfnet.geometry import calculate_dihedral
    ...
    try:
        ...
        if next_res is not None:
            psi_sin, psi_cos, psi_valid = _compute_dihedral_with_mask(
                {**atoms, **{f"next_{k}": v for k, v in next_atoms.items()}},
                ["N", "CA", "C", "next_N"],
            )
        else:
...
class ProteinEncoder:
    ...
    负责从蛋白结构、残基几何和 ESM 信息提取多层级特征，
    生成构图所需的蛋白原子、残基和上下文表示。
    """
    ...
    def encode(
        ...
        *,
        ...
        结合结构几何与 ESM 表征提取蛋白原子、残基和上下文特征，
        输出构图阶段所需的蛋白侧表示。

        Args:
            universe: MDAnalysis Universe 蛋白结构对象。
            esm_embeddings: 按残基索引组织的 ESM 嵌入字典。

        Returns:
            ProteinEncodingResult: 返回蛋白原子、残基和上下文节点所需的完整编码结果。
        ...
        for segment_id, segment in enumerate(residue_segments):
            ...
            for seg_pos, seg_res in enumerate(seg_residues):
                ...
                next_res_by_ix[res_ix] = seg_residues[seg_pos + 1] if seg_pos + 1 < seg_len else None
                ...
                chain_index = chain_label_to_index.setdefault(chain_label, len(chain_label_to_index))
